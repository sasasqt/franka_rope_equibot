        ...
        for i in range(cls.obs_horizon-extra_repeat):
            ...
            if cls.cfg.from_demo is not None:
                ...
                world.scene.get_object('/World/Extras/TCube/TCube/cube').set_world_poses(
                    positions=np.array([np.array(data_frame.data["TCube"]["cube_world_position"])+cls._translation]),
                    orientations=np.array([ori]),
                )
...
def update_action(agent_ac,target,eef,gripper,rel,rpy,eps,cap=None,cup=None,update_ori=True,cfg=None):
    ...
    # if torch.sigmoid(agent_ac[3][3]) <0.5:
    #     gripper.close()
    ...
    # gripper.close()
    print(f"{torch.sigmoid(agent_ac[3][3])} gripper is ?????")
