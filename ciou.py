import json
import glob
import os
import numpy as np


# ---------- Geometry helpers ----------

def quat_to_rot_matrix(q):
    """Quaternion [w, x, y, z] -> 3x3 rotation matrix."""
    q = np.asarray(q, dtype=float)
    n = np.linalg.norm(q)
    if n == 0:
        return np.eye(3)
    w, x, y, z = q / n

    return np.array([
        [1 - 2*(y*y + z*z),     2*(x*y - z*w),         2*(x*z + y*w)],
        [2*(x*y + z*w),         1 - 2*(x*x + z*z),     2*(y*z - x*w)],
        [2*(x*z - y*w),         2*(y*z + x*w),         1 - 2*(x*x + y*y)]
    ], dtype=float)


def points_in_box(points, center, quat, scale):
    """
    Check which points lie inside an oriented box.

    center: (3,)
    quat: [w, x, y, z]
    scale: full side lengths -> half extents = scale / 2
    """
    points = np.asarray(points, dtype=float)
    center = np.asarray(center, dtype=float)
    he = 0.5 * np.asarray(scale, dtype=float)
    R = quat_to_rot_matrix(quat)

    # world -> local
    local = (points - center) @ R

    return np.all(np.abs(local) <= he + 1e-8, axis=-1)


def build_grid(pc_a, pc_b, margin=0.05, resolution=80):
    """
    Build a cubic voxel grid covering both point clouds with some margin.
    """
    pc_a = np.asarray(pc_a, dtype=float)
    pc_b = np.asarray(pc_b, dtype=float)

    all_pts = np.concatenate([pc_a, pc_b], axis=0)
    mins = all_pts.min(axis=0) - margin
    maxs = all_pts.max(axis=0) + margin

    extent = maxs - mins
    max_dim = float(extent.max())
    step = max_dim / float(resolution)

    xs = np.arange(mins[0], maxs[0] + 0.5 * step, step)
    ys = np.arange(mins[1], maxs[1] + 0.5 * step, step)
    zs = np.arange(mins[2], maxs[2] + 0.5 * step, step)

    X, Y, Z = np.meshgrid(xs, ys, zs, indexing='ij')
    grid = np.stack([X.ravel(), Y.ravel(), Z.ravel()], axis=-1)
    return grid


# ---------- Cube-specific helpers ----------

def infer_box_scale_from_pc(center, quat, pc):
    """
    Infer full side lengths (scale) of a box from center/orientation/points.
    """
    pc = np.asarray(pc, dtype=float)
    center = np.asarray(center, dtype=float)
    R = quat_to_rot_matrix(quat)

    local = (pc - center) @ R
    he = np.max(np.abs(local), axis=0)
    eps = 1e-8
    return 2.0 * (he + eps)


def volumetric_iou_cube_for_entry(entry,
                                  resolution=80,
                                  margin=0.05,
                                  target_z_offset=0.0):
    """
    Volumetric IoU between Cube and TCube for one frame.
    Applies a z-offset to TCube.
    """
    C = entry["data"]["Cube"]
    TC = entry["data"]["TCube"]

    pc = np.asarray(C["pc"], dtype=float)
    pc_tgt = np.asarray(TC["pc"], dtype=float)
    if target_z_offset != 0.0:
        pc_tgt = pc_tgt.copy()
        pc_tgt[:, 2] += float(target_z_offset)

    # Grid over both shapes (with offset TCube)
    grid = build_grid(pc, pc_tgt, margin=margin, resolution=resolution)

    # Pred cube
    cube_scale = infer_box_scale_from_pc(
        C["cube_world_position"],
        C["cube_world_orientation"],
        pc,
    )

    # Target cube center + offset
    tc_center = np.array(TC["cube_world_position"], dtype=float)
    tc_center[2] += float(target_z_offset)

    # Target cube scale
    tcube_scale = infer_box_scale_from_pc(
        tc_center,
        TC["cube_world_orientation"],
        pc_tgt,
    )

    occ_C = points_in_box(
        grid,
        C["cube_world_position"],
        C["cube_world_orientation"],
        cube_scale,
    )

    occ_TC = points_in_box(
        grid,
        tc_center,
        TC["cube_world_orientation"],
        tcube_scale,
    )

    inter = np.logical_and(occ_C, occ_TC).sum()
    union = np.logical_or(occ_C, occ_TC).sum()

    if union == 0:
        return 0.0
    return inter / union


# ---------- Metric helpers ----------

def get_start_index_after_first_drop(frames):
    """Index of first frame AFTER current_time_step decreases."""
    if not frames:
        return None

    prev = frames[0]["current_time_step"]
    for i in range(1, len(frames)):
        cur = frames[i]["current_time_step"]
        if cur < prev:
            return i
        prev = cur
    return None


def metric_for_file_cube(path,
                         resolution=80,
                         margin=0.05,
                         min_count=20,
                         target_z_offset=0.0,
                         verbose=False):
    """
    Cube/TCube metric with optional TCube z-offset.
    """
    with open(path, "r") as f:
        data = json.load(f)

    frames = data["Isaac Sim Data"]
    start_idx = get_start_index_after_first_drop(frames)
    if start_idx is None:
        if verbose:
            print(os.path.basename(path), "NO_DROP_FOUND")
        return None, 0

    ious = [
        volumetric_iou_cube_for_entry(
            frames[i],
            resolution=resolution,
            margin=margin,
            target_z_offset=target_z_offset,
        )
        for i in range(start_idx, len(frames))
    ]

    num_frames = len(ious)
    if num_frames < min_count:
        if verbose:
            print(os.path.basename(path),
                  f"NOT_ENOUGH_FRAMES_AFTER_DROP ({num_frames} < {min_count})")
        return None, num_frames

    sorted_ious = sorted(ious, reverse=True)
    tau = sorted_ious[min_count - 1]

    if verbose:
        print(os.path.basename(path), "tau:", tau,
              f"(from {num_frames} frames after drop)")
    return tau, num_frames


# ---------- CLI ----------

if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(
        description="Compute Cube/TCube volumetric IoU metric over JSON logs."
    )
    parser.add_argument(
        "--log-dir",
        type=str,
        required=True,
        help="Directory containing JSON log files."
    )
    parser.add_argument(
        "--min-count",
        type=int,
        default=20,
        help="Minimum number of frames for tau (default: 20)."
    )
    parser.add_argument(
        "--resolution",
        type=int,
        default=80,
        help="Voxel grid resolution along largest dim (default: 80)."
    )
    parser.add_argument(
        "--margin",
        type=float,
        default=0.05,
        help="Spatial margin around point clouds (default: 0.05)."
    )
    parser.add_argument(
        "--target-z-offset",
        type=float,
        default=0.06,
        help="Z offset added to TCube before IoU (default: 0.0)."
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Print per-file details."
    )

    args = parser.parse_args()

    for path in sorted(glob.glob(os.path.join(args.log_dir, "*.json"))):
        tau, n = metric_for_file_cube(
            path,
            resolution=args.resolution,
            margin=args.margin,
            min_count=args.min_count,
            target_z_offset=args.target_z_offset,
            verbose=args.verbose,
        )
        if tau is None:
            print(os.path.basename(path),
                  "-> metric: N/A (used frames:", n, ")")
        else:
            print(os.path.basename(path),
                  "-> metric:", tau, "(used frames:", n, ")")
