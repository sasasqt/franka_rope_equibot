...
# data[0]['Isaac Sim Data'][i]['data'].keys()=dict_keys(['Left', 'Left', 'Rope', 'extras', 'Datetime'])
...
# data[0]['Isaac Sim Data'][i]['data']['extras']=dict_keys(['left_reposition_pressed', 'left_reposition_pressed'])
# recording={}
# recording["pc"]=[]
# recording["action"]=[]
# recording["eef_pos"]=[]
...
def main(cfg):
    ...
    for ep, filename in enumerate(os.listdir(input_dir)):
        ...
        for i, _fut in enumerate(data[0]["Isaac Sim Data"]):
            ...
            delta_pos = (
                random_rotation@np.array(fut["Left"]["Left_target_world_position"])
