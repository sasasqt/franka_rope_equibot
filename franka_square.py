...
class FrankaRope(BaseSample):
    ...
    def _add_square(self):
        ...
        stage=self._world.stage
        self._tshape_xform=_tshape_xform =stage.DefinePrim(f'/World/Extras/Square', 'Xform')
        usd_tshape_xform = UsdGeom.Xform(_tshape_xform)
        usd_tshape_xform.AddTranslateOp().Set(Gf.Vec3f([0,0,0]))
        usd_tshape_xform.AddRotateXYZOp().Set(Gf.Vec3f([0,0,0]))
        usd_tshape_xform.AddScaleOp().Set(Gf.Vec3f([1,1,1]))

        self._square_str=square_str=find_unique_string_name(
                    initial_name=f"/World/Extras/Square/Square", is_unique_fn=lambda x: not is_prim_path_valid(x)
                )
        ...
        omni.kit.commands.execute('SetRigidBody',
            path=Sdf.Path('/World/Extras/Square/Square/hollow_square_handle'),
    ...
    async def _on_replay_recording_event_async(self, data_file,callback_fn=None):
        ...
        world.add_physics_callback("replay_recording", partial(self._on_replay_square_recording_step,time_offset=world.current_time_step_index))
    ...
    def _on_replay_square_recording_step(self, step_size, time_offset=0):
        world=self._world
        data_logger=self._data_logger
        square=self._square
        ...
        if world.current_time_step_index-time_offset < data_logger.get_num_of_data_frames():
            ...
            square.set_world_pose(
                position=np.array(data_frame.data["Square"]["Square_world_position"]),
                orientation=np.array(data_frame.data["Square"]["Square_world_orientation"]),
            )
            # if world.current_time_step_index-time_offset<40:
            print(square.get_world_poses()[0],world.scene.get_object(target_name).get_world_pose()[0],world.scene.get_object(target_name).get_world_pose()[1],world.current_time_step_index-time_offset)            
    ...
    def _on_logging_event(self, val,callback_fn=None,extras_fn=None):
        ...
        if not data_logger.is_started():
            ...
            def frame_logging_func(tasks, scene):
                ...
                dict=_dict["Square"]={
                    "square_world_position": self._square.get_world_poses()[0][0].tolist(),
